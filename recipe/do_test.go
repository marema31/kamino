...
func TestRecipeForceOk(t *testing.T) {
	for _, sequential := range []bool{false, true} {
		...
		for _, step := range sf.Steps["recipe2ok"] {
			if !step.Called {
				t.Errorf("A step of recipe2ok was not called")
			}
		}
	}
}
func TestRecipePostLoadOk(t *testing.T) {
	for _, sequential := range []bool{false, true} {
		...
		err = ck.PostLoad(log, make(map[string]string))
		if err != nil {
			t.Errorf("PostLoad should not return error, returned %v", err)
		}

		for rname := range sf.Steps {
			seen := make(map[bool]int)
			for _, step := range sf.Steps[rname] {
				seen[step.PostLoaded] += 1
			}
			if seen[true] != len(sf.Steps[rname]) {
				t.Errorf("No all step of %s was postloaded", rname)
			}
		}
	}
}
...
func TestRecipePostLoadError(t *testing.T) {
	for _, sequential := range []bool{false, true} {
		...
		err = ck.PostLoad(log, make(map[string]string))
		if err == nil {
			t.Errorf("PostLoad should return error")
		}
	}
}
