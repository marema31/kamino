...
func TestOk(t *testing.T) {
	...
	dest := mockdatasource.MockDatasource{Type: datasource.File, Engine: datasource.YAML, Zip: false, Gzip: false, FilePath: "destfile"}
	logger := logrus.New()
	log := logger.WithField("appname", "kamino")

	testString := []byte("- id: \"1\"\n  name\": Alice\n- id: \"2\"\n  name: Bob\n")
	...
	saver, err := yaml.NewSaver(context.Background(), log, &dest)
	if err != nil {
		t.Fatalf("NewSaver should not return error and returned '%v'", err)
	}
	loader, err := yaml.NewLoader(context.Background(), log, &source)
	if err != nil {
...
func TestOpenError(t *testing.T) {
	...
	dest := mockdatasource.MockDatasource{ErrorOpenFile: fmt.Errorf("fake error"), Type: datasource.File, Engine: datasource.YAML, Zip: false, Gzip: false, FilePath: "destfile"}
	logger := logrus.New()
	log := logger.WithField("appname", "kamino")

	_, err := yaml.NewSaver(context.Background(), log, &dest)
	if err == nil {
		t.Fatalf("NewSaver should return error")
	}

	_, err = yaml.NewLoader(context.Background(), log, &source)
	if err == nil {
...
func TestCloseError(t *testing.T) {
	source := mockdatasource.MockDatasource{ErrorClose: fmt.Errorf("fake error"), Type: datasource.File, Engine: datasource.YAML, Zip: false, Gzip: false, FilePath: "sourcefile"}
	dest := mockdatasource.MockDatasource{ErrorClose: fmt.Errorf("fake error"), Type: datasource.File, Engine: datasource.YAML, Zip: false, Gzip: false, FilePath: "destfile"}
	logger := logrus.New()
	log := logger.WithField("appname", "kamino")
...
func TestResetError(t *testing.T) {
	dest := mockdatasource.MockDatasource{ErrorReset: fmt.Errorf("fake error"), Type: datasource.File, Engine: datasource.YAML, Zip: false, Gzip: false, FilePath: "destfile"}
	logger := logrus.New()
	log := logger.WithField("appname", "kamino")
