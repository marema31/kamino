...
func setupDo(path string, filename string) (context.Context, *logrus.Entry, datasource.Datasourcers, *viper.Viper, *mockprovider.MockProvider) {
	...
	dss.Insert([]string{"tagcachenotexist"}, []datasource.Type{datasource.File}, []datasource.Engine{datasource.JSON}, []*mockdatasource.MockDatasource{&dscachenotexist})
	dss.Insert([]string{"tagerror"}, []datasource.Type{datasource.Database}, []datasource.Engine{datasource.Mysql}, []*mockdatasource.MockDatasource{&ds5})
	dss.Insert([]string{"tagerrorfile"}, []datasource.Type{datasource.File}, []datasource.Engine{datasource.JSON}, []*mockdatasource.MockDatasource{&dserrorfile})
