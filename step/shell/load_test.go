...
func TestShellLoadOk(t *testing.T) {
	...
	priority, steps, err := shell.Load(ctx, log, "testdata/good/", "nameshellok", 0, v, dss)
	if err != nil {
...
func TestShellLoadNoTag(t *testing.T) {
	...
	priority, steps, err := shell.Load(ctx, log, "testdata/good/", "notags", 0, v, dss)
	if err != nil {
		t.Errorf("Load should not returns an error, returned: %v", err)
	}
...
func TestShellLoadNoScript(t *testing.T) {
	...
	_, _, err = shell.Load(ctx, log, "testdata/fail", "noscript", 0, v, dss)
	if err == nil {
...
func TestShellLoadTemplateWrong(t *testing.T) {
	...
	_, _, err = shell.Load(ctx, log, "testdata/fail", "wrongtemplate", 0, v, dss)
	if err == nil {
		t.Errorf("Load should returns an error")
	}
}
...
func TestShellLoadWrongEngine(t *testing.T) {
	...
	_, _, err = shell.Load(ctx, log, "testdata/fail", "wrongengine", 0, v, dss)
	if err == nil {
		t.Errorf("Load should returns an error")
	}
}
...
func TestShellPostLoadOk(t *testing.T) {
	...
	_, steps, err := shell.Load(ctx, log, "testdata/good/", "nameshellok", 0, v, dss)
	if err != nil {
		t.Errorf("Load should not returns an error, returned: %v", err)
	}
