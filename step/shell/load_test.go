...
func TestShellLoadOk(t *testing.T) {
	...
	priority, steps, err := shell.Load(ctx, log, "testdata/good/", "nameshellok", 0, v, dss, false)
	if err != nil {
...
func TestShellLoadNoTag(t *testing.T) {
	...
	priority, steps, err := shell.Load(ctx, log, "testdata/good/", "notags", 0, v, dss, false)
	if err != nil {
		t.Errorf("Load should not returns an error, returned: %v", err)
	}
...
func TestShellLoadNoScript(t *testing.T) {
	...
	_, _, err = shell.Load(ctx, log, "testdata/fail", "noscript", 0, v, dss, false)
	if err == nil {
...
func TestShellLoadWrongEngine(t *testing.T) {
	...
	_, _, err = shell.Load(ctx, log, "testdata/fail", "wrongengine", 0, v, dss, false)
	if err == nil {
		t.Errorf("Load should returns an error")
	}
}
